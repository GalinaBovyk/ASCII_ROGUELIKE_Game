...
class FatigueDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction)-> None:
        ...
        if target:
            self.engine.message_log.add_message(
                f"You read out a list of the upcomping deadlines. A wave of exhaustion washes over {target.name} and it takes {self.damage} damage."
            )
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        for actor in self.engine.game_map.actors:
            if actor.distance(*target_xy) <=self.radius:
                self.engine.message_log.add_message(
                    f"A Stink Cloud engilfs the {actor.name}, making it cough out its lungs for {self.damage} damge!"
                )
